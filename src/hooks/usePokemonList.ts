...
export function usePokemonList(limit: number = 20): UsePokemonListReturn {
  ...
  const loadPokemons = useCallback(
    async (currentOffset: number, isRefresh = false) => {
      ...
      try {
        ...
        console.log("Fetched pokemon list:", listResponse);
  ...
  }, [limit, hasMore, loading, loadPokemons]);
...
export function usePokemonInfo(
  ...
  const fetchPokemonInfo = useCallback(async () => {
    ...
    try {
      console.log("Fetching pokemon info for ID:", pokemonId);
      ...
      console.log("Pokemon info loaded successfully:", transformedPokemon);
    } catch (err) {
      const errorMessage =
